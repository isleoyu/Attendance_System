...
/**
 * GET /api/reports/daily?storeId=xxx&startDate=xxx&endDate=xxx
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = await reportService.getDailyAttendanceStats(
      storeId,
      new Date(startDate),
      new Date(endDate)
    )
    ...
    return NextResponse.json({ error: "取得每日統計失敗" }, { status: 500 })
  }
}
